...
extern "C"
{
  ...
  const char s_szGN7[] PROGMEM = "Terrain_Ripple 12"
};
#endif
/**
typedef struct _PhoenixGait {
  ...
  {NomGaitSpeed, StepsInGait, NrLiftedPos, FrontDownPos, LiftDivFactor, TLDivFactor, HalfLiftHeight, NAME}
**/
PHOENIXGAIT APG[] = {
    ...
    //New gait here
    {DEFAULT_SLOW_GAIT, 12, 3, 2, 2, 8, 3, {7, 11, 3, 1, 5, 9} GAITNAME(s_szGN7)} // Terrain Tripod Gait
};
...
extern void Gait_Terrain(byte GaitCurrentLegNr);
...
void loop(void)
{
  ...
  if (g_InControlState.BalanceMode)
  {
    ...
    if (g_fDebugOutput)
    {
      TravelRequest = (abs(g_InControlState.TravelLength.x) > cTravelDeadZone) || (abs(g_InControlState.TravelLength.z) > cTravelDeadZone) || (abs(g_InControlState.TravelLength.y) > cTravelDeadZone) || (g_InControlState.ForceGaitStepCnt != 0) || fWalking;
  ...
  if (g_InControlState.fRobotOn)
  {
    ...
    for (LegIndex = 0; LegIndex < CNT_LEGS; LegIndex++)
    {
      if ((GaitPosX[LegIndex] > cGPlimit) || (GaitPosX[LegIndex] < -cGPlimit) || (GaitPosZ[LegIndex] > cGPlimit) || (GaitPosZ[LegIndex] < -cGPlimit) || (GaitRotY[LegIndex] > cGPlimit) || (GaitRotY[LegIndex] < -cGPlimit))
      {
...
void GaitSeq(void)
{
  ...
  //7 is the index of the new terrain gait
  if (g_InControlState.GaitType != 7)
  {
...
void Gait(byte GaitCurrentLegNr)
{
  ...
  if ((TravelRequest && (g_InControlState.gaitCur.NrLiftedPos & 1) &&
       LegStep == 0) ||
      (!TravelRequest && LegStep == 0 && ((abs(GaitPosX[GaitCurrentLegNr]) > 2) || (abs(GaitPosZ[GaitCurrentLegNr]) > 2) || (abs(GaitRotY[GaitCurrentLegNr]) > 2))))
  { //Up
...
//--------------------------------------------------------------------
//[GAIT] Terrain
// This is the alternate gait sequence for terrain navigation
// The difference between this and the other gait is the use of the microswitches and angle reading
void Gait_Terrain(byte GaitCurrentLegNr)
{
...
void BodyFK(short PosX, short PosZ, short PosY, short RotationY, byte BodyIKLeg)
{
  ...
  BodyFKPosX = ((long)CPR_X * c2DEC - ((long)CPR_X * c2DEC * CosA4 / c4DEC * CosB4 / c4DEC - (long)CPR_Z * c2DEC * CosB4 / c4DEC * SinA4 / c4DEC + (long)CPR_Y * c2DEC * SinB4 / c4DEC)) / c2DEC;
  BodyFKPosZ = ((long)CPR_Z * c2DEC - ((long)CPR_X * c2DEC * CosG4 / c4DEC * SinA4 / c4DEC + (long)CPR_X * c2DEC * CosA4 / c4DEC * SinB4 / c4DEC * SinG4 / c4DEC + (long)CPR_Z * c2DEC * CosA4 / c4DEC * CosG4 / c4DEC - (long)CPR_Z * c2DEC * SinA4 / c4DEC * SinB4 / c4DEC * SinG4 / c4DEC - (long)CPR_Y * c2DEC * CosB4 / c4DEC * SinG4 / c4DEC)) / c2DEC;
  BodyFKPosY = ((long)CPR_Y * c2DEC - ((long)CPR_X * c2DEC * SinA4 / c4DEC * SinG4 / c4DEC - (long)CPR_X * c2DEC * CosA4 / c4DEC * CosG4 / c4DEC * SinB4 / c4DEC + (long)CPR_Z * c2DEC * CosA4 / c4DEC * SinG4 / c4DEC + (long)CPR_Z * c2DEC * CosG4 / c4DEC * SinA4 / c4DEC * SinB4 / c4DEC + (long)CPR_Y * c2DEC * CosB4 / c4DEC * CosG4 / c4DEC)) / c2DEC;
}
